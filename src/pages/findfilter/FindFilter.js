...
const FindFilter = () => {
	...
	return (
		<section>
			<PageBanner destination={userInput.destination} month={userInput.month} type={userInput.type} title="Package For Your Choice"/>
								...
								{numberOfPages &&
									numberOfPages.map((number, index) => (
										<li
											...
											key={index}
										>
											<a className="page-link"  onClick={() => changeCurrentPage(number)}>
												{number}
											</a>
										</li>
									))}
