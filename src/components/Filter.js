...
const Filter = () => {
  ...
  const handleClick = () => {
    navigate("/findfilter", {state: {filteredTour: filteredTour, destination: selectedDest, month: selectedMonth, type: selectedTravel}});
  }
  ...
  return (
    <Container className='filter__sec py-4 px-5'>
      ...
      {capitalizeddest.map((item,index)=>(
        <Dropdown.Item key={index} onClick={(e)=> handleDestination(e)}>{item}</Dropdown.Item>
      ))}
      </Dropdown.Menu>
    </Dropdown>
        </Col>
      ...
      <Dropdown.Menu>
        ...
        <Dropdown.Item key={index} onClick={(e)=> handleMonth(e)}>{item}</Dropdown.Item>
      ))}
      ...
      <Dropdown.Menu>
        ...
        <Dropdown.Item key={index} onClick={(e)=>handleTravel(e)}>{item}</Dropdown.Item>
      ))}
